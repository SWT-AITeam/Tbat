...
class TMR_RE(nn.Module):
    def __init__(self, max_length, pretrain_path, blank_padding=True, mask_entity=False, IB=False):
        """
        ...
        self.linear_pic = nn.Linear(2048, self.hidden_size )
        self.linear_final = nn.Linear(self.hidden_size * 3+self.hidden_size, self.hidden_size)
        # self.bn = nn.BatchNorm1d(self.hidden_size * 3)
        # the attention mechanism for entity description features
        self.linear_q_entity = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        # self.linear_k_entity = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        # self.linear_v_entity = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)

        # the attention mechanism for fine-grained features
        # self.linear_q_fine = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        ...
        # nn.LSTM
        # self.seq_logvar_layer = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        # self.IB = IB
        # self.kl_loss = nn.KLDivLoss(reduction="batchmean")
        # print(f"activate IB: {self.IB}")

    def forward(self, token, att_mask, pos1, pos2, token_phrase, att_mask_phrase, h_des_token, h_des_mask, t_des_token,
                t_des_mask, image_diff, image_ori, image_diff_objects, image_ori_objects, weights):
        """
        Args:
        ...
        output_h_des = self.bert(h_des_token, attention_mask=h_des_mask)
        hidden_h_des = output_h_des[1]
        output_t_des = self.bert(t_des_token, attention_mask=t_des_mask)
        hidden_t_des = output_t_des[1]

        # hidden_k_text = self.linear_k_entity(hidden_text)
        # hidden_v_text = self.linear_v_entity(hidden_text)
        hidden_q_h_des = self.linear_q_entity(hidden_h_des)
        # h_fussion = torch.sum(torch.tanh(self.att(hidden_q_h_des, hidden_k_text, hidden_v_text)), dim=1)
        # # hidden_k_text = self.linear_k_entity(hidden_text)
        # # hidden_v_text = self.linear_v_entity(hidden_text)
        hidden_q_t_des = self.linear_q_entity(hidden_t_des)
        # t_fussion = torch.sum(torch.tanh(self.att(hidden_q_t_des, hidden_k_text, hidden_v_text)), dim=1)
        ...
        head_hidden += hidden_q_h_des
        tail_hidden += hidden_q_t_des
        # x = torch.cat([head_hidden, tail_hidden, hidden_phrases, pic_ori, pic_diff], dim=-1)
        x = torch.cat([head_hidden, tail_hidden, hidden_phrases,pic_ori_, hidden_q_h_des, hidden_q_t_des], dim=-1)
        # x = self.bn(x)
        # x = torch.cat([head_hidden, tail_hidden, hidden_phrases], dim=-1)
        # x = torch.cat([head_hidden, tail_hidden, hidden_q_h_des, hidden_q_t_des], dim=-1)
        # x = torch.stack([head_hidden,tail_hidden,hidden_phrases, pic_ori, pic_diff], dim=1)
        # x = torch.stack([head_hidden,tail_hidden], dim=1)
        # x,kld = self._variational_layer(x,self.seq_logvar_layer)
        # bs,device = x.shape[0],x.device
        # x = self.linear_final(self.dropout_linear(x.reshape(bs,-1)))
        x = self.linear_final(self.dropout_linear(x))
        # l = torch.matmul(kld, torch.ones(bs,5).T.type(torch.float32).to(device)).sum()
        # print(x)
    ...
    def tokenize(self, item):
        ...
        h_des_token = self.tokenizer.convert_tokens_to_ids(['[CLS]'] + self.tokenizer.tokenize(item['h_des']))
        while len(h_des_token) < 30:
            h_des_token.append(0)
        h_des_token = h_des_token[:30]
        h_des_mask = torch.zeros(1, 30)
        h_des_mask[0, :len(h_des_token)] = 1
        h_des_token = torch.tensor(h_des_token).long().unsqueeze(0)
        t_des_token = self.tokenizer.convert_tokens_to_ids(['[CLS]'] + self.tokenizer.tokenize(item['t_des']))
        while len(t_des_token) < 30:
            t_des_token.append(0)
        t_des_token = t_des_token[:30]
        t_des_mask = torch.zeros(1, 30)
        t_des_mask[0, :len(h_des_token)] = 1
        t_des_token = torch.tensor(t_des_token).long().unsqueeze(0)
        return indexed_tokens, att_mask, pos1, pos2, token_phrases, att_mask_phrases, h_des_token, h_des_mask, t_des_token, t_des_mask
