...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, text_path, pic_path, rel2id, tokenizer, sample_ratio, kwargs):
        ...
        self.entity_description_path = text_path.replace('ours_{}.txt'.format(mode), "entity_describe.json")
        self.description_data = json.load(open(self.entity_description_path, "r"))
    ...
    def __getitem__(self, index):
        ...
        for i in range(3):
            ...
            if len(self.state_dict_ori[index]) > i:
                image_ori_object = Image.open(
                    os.path.join(self.pic_path_CoarseGrained_ori, 'crops/' + self.state_dict_ori[index][i])).convert(
                    'RGB')
        ...
        h = item['h']['name']  # head entity
        t = item['t']['name']  # tail entity
        h_des = self.description_data[h]
        t_des = self.description_data[t]
        ...
        res = [self.rel2id[item['relation']]] + [img_id] + seq + list_p1 + list_p2 + list_p3 + list_p4 + [
            torch.tensor(weight).reshape(1, 4)]
